...
func Test_FindPluginPath(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			Expected: filepath.Join(PluginRoot, "tflint-ruleset-foo"),
		},
		{
			Name: "auto installed",
			...
			Expected: filepath.Join(PluginRoot, "github.com/terraform-linters/tflint-ruleset-bar", "0.1.0", "tflint-ruleset-bar"),
		},
	}
...
func Test_FindPluginPath_locals(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			Name:     "manually installed",
			Input:    NewInstallConfig(&tflint.PluginConfig{Name: "foo", Enabled: true}),
			Expected: filepath.Join(localPluginRoot, "tflint-ruleset-foo"),
		},
		{
			Name: "auto installed",
			Input: NewInstallConfig(&tflint.PluginConfig{
			...
			Expected: filepath.Join(localPluginRoot, "github.com/terraform-linters/tflint-ruleset-bar", "0.1.0", "tflint-ruleset-bar"),
		},
	}
...
func Test_FindPluginPath_envVar(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			Name:     "manually installed",
			Input:    NewInstallConfig(&tflint.PluginConfig{Name: "foo", Enabled: true}),
			Expected: filepath.Join(dir, "tflint-ruleset-foo"),
		},
		{
			Name: "auto installed",
			Input: NewInstallConfig(&tflint.PluginConfig{
			...
			Expected: filepath.Join(dir, "github.com/terraform-linters/tflint-ruleset-bar", "0.1.0", "tflint-ruleset-bar"),
		},
	}
